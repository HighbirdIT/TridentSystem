...
class Extractor(object):
    ...
    def _find_cross_points(self, hlines, vlines):   #计算点位
        points = []
        delta = 1
        for vline in vlines:
            vx0 = vline['x0']
    ...
    def _find_outer(self, k, words,res):
        df = pd.DataFrame()
    ...
    def _find_inner(self, k, words, groups, groups2, res, free_zone_flag=False):
        df = pd.DataFrame()
        ...
        if '购买方' in context or '销售方' in context:
            ...
            for pos, text in group_context.items():
                if '名称' in text:
                    ...
                    res[prefix+'纳税人识别号']=tax_man_id
                elif '地址、电话' in text:
                    addr = re.sub(r'地址、电话:', '', text)
                    df.loc[0, prefix+'地址电话'] = addr
                    res[prefix+'地址电话']=addr
                elif '开户行及账号' in text:
                    account = re.sub(r'开户行及账号:', '', text)
                    df.loc[0, prefix+'开户行及账号'] = account
                    res[prefix+'开户行及账号']=account
        elif '项目名称' in context:
            name = re.sub(r'项目名称', '', context)
            df.loc[0, '项目名称'] = name
            res['项目名称']=name
        elif '车牌号' in context:
            name = re.sub(r'车牌号', '', context)
            df.loc[0, '车牌号'] = name
            res['车牌号']=name    
        elif '类型' in context:
            name = re.sub(r'类型', '', context)
            df.loc[0, '类型'] = name
            res['类型']=name
        elif '通行日期起' in context:
            name = re.sub(r'通行日期起', '', context)
            df.loc[0, '通行日期起'] = name
            res['通行日期起']=name   
        elif '通行日期止' in context:
            name = re.sub(r'通行日期止', '', context)
            df.loc[0, '通行日期止'] = name
            res['通行日期止']=name
        elif '金额' in context:
            name = re.sub(r'金额', '', context)
            idex = name.find("￥")
            if idex >= 0:
                df.loc[0, '金额'] = name[0:idex]
                res['金额']=name[0:idex]
            else:
                df.loc[0, '金额'] = name
                res['金额'] = name
        elif '税率' in context:
            name = re.sub(r'税率', '', context)
            df.loc[0, '税率'] = name
            res['税率']=name   
        elif '税额' in context:
            name = re.sub(r'税额', '', context)
            idex = name.find("￥")
            if idex >= 0:
                df.loc[0, '税额'] = name[0:idex]
                res['税额']=name[0:idex]
            else:
                df.loc[0, '税额'] = name
                res['税额'] = name
    ...
    def extract(self):
        ...
        for k, words in word_groups2.items():
            if k[0] == 'OUT':
                df_item = self._find_outer(k, words,res)
            else:
                df_item, free_zone_flag = self._find_inner(
                    k, words, word_groups, word_groups2, res, free_zone_flag)
...
#             worksheet.write(row, column, data[key])
#             stringDate += data[key] + ','
#             column = column + 1
#         print(stringDate[:-1] + ')')
        
#         row = row + 1
#     workbook.save(r'D:/通行费电子发票/formatting.xls') # 保存文件
#     #worksheet.write(0, 0, data['发票名称']) # 不带样式的写入
