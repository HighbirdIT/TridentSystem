    ...
    compile(helper, preNodes_arr) {
        ...
        for (var i = 0; i < this.inputScokets_arr.length; ++i) {
            ...
            if (tLinks.length == 0) {
                ...
                var outNode = link.outSocket.node;
                var compileRet = outNode.compile(helper, usePreNodes_arr);
                var LiketypeValue=theSocket.getExtra('LiketypeValue')
                if (compileRet == false) {
                    return false;
                }
                inScoketValue = compileRet.getSocketOut(link.outSocket).strContent;//link.outSocket
                socketVal_arr.push(inScoketValue);
            }
            socketVal_arr.push(tValue);
        }
        var finalStr = '';
