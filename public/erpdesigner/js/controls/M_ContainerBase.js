...
function M_ContainerBase_tryPlaceKernel(theKernel, mousePos) {
    ...
    if (MyMath.isPointInRect(childContainer_cr, mousePos)) {
        //console.log('命中');
        var placedInChildren = selfKernel.children.find(function (childData) {
        ...
        if (!placedInChildren) {
            if (theKernel.parent == selfKernel) {} else if (theKernel.banReParent || selfKernel.staticChild || !selfKernel.canAppand(theKernel)) {
                return false;
            }
            ...
            for (var ci = 0; ci < selfKernel.children.length; ++ci) {
                ...
                if (childRootElem == null) continue;
