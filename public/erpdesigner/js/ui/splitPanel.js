    ...
    _createClass(SplitPanel, [{
        ...
        value: function render() {
            ...
            return React.createElement(
                'div',
                { className: "flex-grow-1 flex-shrink-1 d-flex" + (!isHor ? ' flex-column mw-100' : ' mh-100'), ref: this.rootDivRef },
                !hideOne && React.createElement(
                    'div',
                    { className: 'hidenOverflow d-flex ' + (fixedOne && !hideTwo ? ' flex-grow-0 flex-shrink-0' : 'flex-grow-1 flex-shrink-1'), style: !fixedOne ? {} : isHor ? { width: usePercent } : { height: usePercent } },
                    this.props.panel1
                ),
                React.createElement(
                    'div',
                    { onMouseDown: this.mouseDownSplitHandler, className: 'text-justify d-flex flex-grow-0 flex-shrink-0 ' + (isHor ? ' splitbar_H align-items-center' : ' splitbar_V justify-content-center') + (this.props.barClass ? ' ' + this.props.barClass : '') },
                    React.createElement('span', { className: 'icon-sm text-light ' + (isHor ? 'icon-more-vertical' : 'icon-more') })
                ),
                !hideTwo && React.createElement(
                    'div',
                    { className: 'hidenOverflow d-flex ' + (fixedOne || hideOne ? ' flex-grow-1 flex-shrink-1' : 'flex-grow-0 flex-shrink-0'), style: fixedOne ? {} : isHor ? { width: usePercent } : { height: usePercent } },
                    this.props.panel2
                )
            );
        }
