...
function createHostRootFiber(isConcurrent) {
  ...
  if (enableProfilerTimer && isDevToolsPresent) {
    ...
    // This enables DevTools to start capturing timing at any point¨C
...
// can be mutated and processed asynchronously before it is committed ¡ª a form
...
function enqueueUpdate(fiber, update) {
  ...
  if (queue2 === null || queue1 === queue2) {
    ...
    // while accounting for the persistent structure of the list ¡ª we don't
  ...
  function warnOnInvalidKey(child, knownKeys) {
    {
      ...
      switch (child.$$typeof) {
        ...
        case REACT_PORTAL_TYPE:
          ...
          warning$1(false, 'Encountered two children with the same key, `%s`. ' + 'Keys should be unique so that components maintain their identity ' + 'across updates. Non-unique keys may cause children to be ' + 'duplicated and/or omitted ¡ª the behavior is unsupported and ' + 'could change in a future version.', key);
          break;
        default:
          break;
      }
...
function updateSuspenseComponent(current$$1, workInProgress, renderExpirationTime) {
  ...
  // the fallback children ¡ª don't need to preserve their state.
  //
...
function commitRoot(root, finishedWork) {
  ...
  if (enableSchedulerTracing) {
    ...
    try {
      ...
      // In which case¨C leave pending interactions until the subsequent render.
