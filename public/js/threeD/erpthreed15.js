...
var C构件上传记录 = function () {
    ...
    _createClass(C构件上传记录, [{
        key: 'set',
        value: function set(上传记录代码, 构件编号, 构件全称, 文件路径, 上传时间, 构件代码, worldPos, 生存状态代码) {
            this.上传记录代码 = 上传记录代码;
            this.构件编号 = 构件编号;
            this.构件全称 = 构件全称;
            this.上传时间 = 上传时间;
        ...
        key: 'add\u590D\u6838\u53C2\u6570',
        value: function add(rcd) {
            for (var si in this.复核参数_arr) {
                var 参数item = this.复核参数_arr[si];
...
var componentModelMat = new THREE.MeshStandardMaterial({ color: 0x052C6E, side: THREE.DoubleSide });
var 未到货构件Mat = new THREE.MeshStandardMaterial({ color: 0x052C6E, side: THREE.DoubleSide });
var 已到货构件Mat = new THREE.MeshStandardMaterial({ color: 0x6A92D4, side: THREE.DoubleSide });
var 已安装构件Mat = new THREE.MeshStandardMaterial({ color: 0x8EEB00, side: THREE.DoubleSide });
...
function ThreeD_Load全局模型(callback) {
    var reSetCamera = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
    ...
    if (this.state.showingModelPath != this.state.drawing.全局模型文件路径) {
        var 全局模型文件路径 = this.state.drawing.全局模型文件路径;
        var 全局模型文件代码 = this.state.drawing.全局模型文件代码;
        ...
        if (labelsRefDiv != null) {
            self.projLabelItem_arr.forEach(function (item) {
        ...
        if (this.projModelObject_cache[全局模型文件代码] != null) {
            var cache = this.projModelObject_cache[全局模型文件代码];
        ...
        this.load3dm(全局模型文件路径, '加载图纸模型', 0, 0.8, function (object) {
            self.setState({
                fetch_title: '解析图纸模型'
            });
            try {
...
function shotParam(名称显示, 发图记录代码, 参数上传记录代码, completeCallBack) {
    ...
    popPersistentPage('M_Page_ParamShoter', function () {
        return React.createElement(VisibleCM_Page_ParamShoter, { key: 'M_Page_ParamShoter', id: 'M_Page_ParamShoter', parenPath: 'M_Page_ParamShoter', flowCode: '285' });
    });
}
...
function shotComParam(构件代码, 参数上传数据代码, 图纸种类代码, completeCallBack) {
    ...
    popPersistentPage('M_Page_ParamShoter_com', function () {
        return React.createElement(VisibleCM_Page_ParamShoter, { key: 'M_Page_ParamShoter_com', id: 'M_Page_ParamShoter_com', parenPath: 'M_Page_ParamShoter_com', flowCode: '285' });
    });
}
...
function shotMultiComParam(构件代码, 参数上传数据代码, 图纸种类代码, completeCallBack) {
    ...
    popPersistentPage('M_Page_ParamShoter_comMul', function () {
        return React.createElement(VisibleCM_Page_ParamShoter, { key: 'M_Page_ParamShoter_comMul', id: 'M_Page_ParamShoter_comMul', parenPath: 'M_Page_ParamShoter_comMul', flowCode: '289' });
    });
}
...
function init_M_Page_ParamShoter_ComMul(state, parentPageID) {
    ...
    setTimeout(function () {
        M_Page_ParamShoter_ComMul_onLoad();
    }, 50);
    ...
    _createClass(CM_Page_ParamShoter, [{
        ...
        value: function renderContent() {
            ...
            retElem = React.createElement(
                'div',
                { className: 'd-flex flex-grow-1 flex-shrink-1 flex-column fixPageContent ' },
                React.createElement(VisibleERPC_IFrame, { className: 'flex-grow-1 flex-shrink-1 d-flex flex-column ', proj: 'GJYM', flowCode: this.props.flowCode, pageType: 'mb', onMessageFun: iframe_shot_onReceiveMsg, id: 'iframe_shot', parentPath: this.props.parenPath }),
                this.renderSidePage()
            );
            return retElem;
        }
    ...
    _createClass(ERPC_ThreeDApp_B, [{
        ...
        value: function Changed(want构件记录) {
            ...
            if (want构件记录 == null) {
                ...
                var 图纸代码 = this.state.drawing.code;
                var 部位位置信息 = this.focus构件记录Btn.位置信息;
        ...
        value: function clickCloseDSHandler(ev) {
            ...
            var preDrawing = this.preDrawing;
            setTimeout(function () {
                _this22.setState({ drawing: preDrawing });
            }, 10);
        }
    }, {
        key: 'download\u6784\u4EF6\u53C2\u6570',
        ...
        value: function renderFrame(gameTime) {
            ...
            this.paramBtns_arr.forEach(function (btnItem) {
                if (_this23.focusParam != null) {
                    btnItem.docElem.style.display = 'none';
                    return;
                }
                if (btnItem.record.拍照状态 == 1) {
                    if (_this23.state.doneVisible == false) {
                        btnItem.docElem.style.display = 'none';
                        return;
                    }
                } else {
                    if (_this23.state.undoneVisible == false) {
                        btnItem.docElem.style.display = 'none';
                        return;
                    }
                }
        ...
        value: function render() {
            ...
            if (this.state.showingProjectCode != this.props.projectCode) {
                ...
                if (this.drawingDataDirted) {
                    this.drawingDataDirted = false;
                    setTimeout(function () {
                        pull_DrawingSelectorForm(null, self.props.fullParentPath, true, _this24.props.projectCode, 'pulldata_可选构件安装图纸');
                    }, 10);
                }
            } else {
                var drawingBtn = React.createElement(
                            ...
                            if (this.state.focus构件记录.复核参数_arr.length > 0) {
                                ...
                                extraUIs_arr.push(React.createElement(
                                    ...
                                    this.state.focus构件记录.复核参数_arr.map(function (item) {
                                        return React.createElement(
                                            'button',
                                            { key: item.参数上传数据代码, record_id: item.参数上传数据代码, onClick: _this24.clickShotHandler, className: "btn flex-grow-0 flex-shrink=0 mt-1 btn-" + (item.拍照状态 ? 'success' : 'light') },
                                            React.createElement('i', { className: 'fa fa-camera' }),
                                            item.参数名称
                                        );
                                    })
...
var CM_Page_ComSel = function (_React$PureComponent9) {
    ...
    function CM_Page_ComSel(props) {
        ...
        var _this25 = _possibleConstructorReturn(this, (CM_Page_ComSel.__proto__ || Object.getPrototypeOf(CM_Page_ComSel)).call(this, props));

        _this25.id = 'M_Page_ComSel';
        ERPC_Page(_this25);
        return _this25;
    }
...
var ERPC_ThreeDApp_C = function (_React$PureComponent10) {
    _inherits(ERPC_ThreeDApp_C, _React$PureComponent10);

    function ERPC_ThreeDApp_C(props) {
        _classCallCheck(this, ERPC_ThreeDApp_C);

        var _this26 = _possibleConstructorReturn(this, (ERPC_ThreeDApp_C.__proto__ || Object.getPrototypeOf(ERPC_ThreeDApp_C)).call(this));

        autoBind(_this26);
        ERPControlBase(_this26);
        ERP_ThreeDAppBase(_this26, {
            drawingCode: 0,
            showingModelPath: null,
            showing图元代码: null,
            doneVisible: true,
        ...
        _this26.drawingDataDirted = true;
        _this26.图纸部位Btns_arr = [];
        _this26.focus构件记录 = null;
        _this26.focusParam = null;
        _this26.cameraInfo_arr = [];
        _this26.部位相机loc = {};
        _this26.关联构件模型_dic = {};
        _this26.构件上传记录_arr = [];
        _this26.构件上传记录_map = {};
        _this26.comLabelItems_arr = [];

        _this26.ProjectChanged = ProjectChanged1.bind(_this26);
        _this26.initApp = InitApp1.bind(_this26);
        return _this26;
    }

    _createClass(ERPC_ThreeDApp_C, [{
        key: 'download\u6784\u4EF6\u6A21\u578B',
        value: function download() {
        ...
        value: function targetRecordChanged() {
            ...
            nativeFetchJson(false, threeDServerUrl, { bundle: { 上传记录代码: 上传记录代码 }, action: 'pulldata_查询构件记录图模' }).then(function (json) {
                if (self.props.targetRecordID != 上传记录代码) {
                    return; // 又切换了目录
                }
                var record = json.data[0];
                var newDrawing = { name: record.全局图纸名称, code: record.全局图纸定义代码, 关联模型代码: record.可视模型代码, 参数信息文件路径: record.参数信息文件路径, 参数上传记录代码: record.参数上传记录代码, 全局模型文件代码: record.模型文件代码, 全局模型文件路径: record.模型文件路径 };

                nativeFetchJson(false, threeDServerUrl, { bundle: { 上传记录代码: 上传记录代码, 最大距离: self.props.maxDistance }, action: 'pulldata_查找相邻构件信息' }).then(function (json) {
                    if (self.props.targetRecordID != 上传记录代码) {
                        return; // 又切换了目录
                    }
                    if (json.err != null) {
        ...
        value: function renderFrame(gameTime) {
            ...
            this.comLabelItems_arr.forEach(function (btnItem) {
                if (_this28.state.comLabelVisible == false) {
                    btnItem.docElem.style.display = 'none';
                    return;
                }
                SmartUpdateButtonItem(btnItem, camera, canvas, 1000);
        ...
        value: function render() {
            ...
            if (this.props.projectCode == null) {
            ...
            } else if (this.state.drawing == null) {} else {
                var loadParamModel = (this.state.loadParamModel == null ? this.props.loadParamModel : this.state.loadParamModel) == true;
                topBtnsDiv = React.createElement(
                    'div',
                    ...
                    React.createElement(
                        'button',
                        { className: "btn flex-grow-0 flex-shrink=0 mt-1 btn-" + (!loadParamModel ? 'light' : 'danger'), onClick: this.clickToggleLoadParamLineHandler },
                        !loadParamModel ? '加载参数线信息' : '禁用参数线信息'
                    )
                );
                ...
                if (this.state.fetching == false) {
                    ...
                    extraUIs_arr.push(React.createElement(
                        ...
                        React.createElement(
                            'button',
                            { onClick: this.toggleParamLineVisible, className: 'btn btn-sm btn-light flex-grow-0 flex-shrink=0 mt-1' },
                            React.createElement('i', { className: "fa fa-eye" + (this.state.paramlineVisible == false ? '-slash text-danger' : '') }),
                            '\u53C2\u6570\u7EBF'
                        ),
                        React.createElement(
                            'button',
                            { onClick: this.toggleComLabelVisible, className: 'btn btn-sm btn-light flex-grow-0 flex-shrink=0 mt-1' },
                            React.createElement('i', { className: "fa fa-eye" + (this.state.comLabelVisible == false ? '-slash text-danger' : '') }),
                            '\u6784\u4EF6\u540D\u79F0'
                        )
                    ));
                }
