...
var PositionInfo = function PositionInfo(全局名称, 全局代码, 中文方位, 英文方位, 部位代码, 一级序号, 二级序号) {
    ...
    _createClass(ERPC_ThreeDApp, [{
        ...
        value: function Changed() {
            ...
            if (this.state.showing参数信息文件路径 != this.state.drawing.参数信息文件路径) {
                ...
                var self = this;
                nativeFetchJson(false, threeDServerUrl, { bundle: { 图纸代码: this.state.drawing.code }, action: 'pulldata_全局参数数据' }).then(function (json) {
                    if (json.err != null) {
                        self.setState({
                            fetch_error: '\u51FA\u9519\u4E86:' + JSON.stringify(json.err)
                        });
