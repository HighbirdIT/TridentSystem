...
var C并网调节端 = function C并网调节端() {
    _classCallCheck(this, C并网调节端);
};
...
var ERPC_ThreeDApp_BingWang = function (_React$PureComponent) {
    _inherits(ERPC_ThreeDApp_BingWang, _React$PureComponent);

    function ERPC_ThreeDApp_BingWang(props) {
        _classCallCheck(this, ERPC_ThreeDApp_BingWang);

        var _this2 = _possibleConstructorReturn(this, (ERPC_ThreeDApp_BingWang.__proto__ || Object.getPrototypeOf(ERPC_ThreeDApp_BingWang)).call(this));
    ...
    _createClass(ERPC_ThreeDApp_BingWang, [{
        key: 'toggle\u7F16\u53F7Visible',
        ...
        value: function render() {
            ...
            var netName = '请选择索网';
            if (this.state.targetNetA.code == 0) {
                setTimeout(function () {
                    popBingWangSelector(self.网片选择Callback);
                }, 10);
            } else {
                netName = this.state.targetNetA.name + '-' + this.state.targetNetB.name;
            }
    ...
    return ERPC_ThreeDApp_BingWang;
}(React.PureComponent);

function ERPC_ThreeDApp_BingWang_mapstatetoprops(state, ownprops) {
...
function ERPC_ThreeDApp_BingWang_dispatchtorprops(dispatch, ownprops) {
    return {};
}
var VisibleERPC_ThreeDApp_BingWang = null;

function InitThreedApp_BingWang() {
    VisibleERPC_ThreeDApp_BingWang = ReactRedux.connect(ERPC_ThreeDApp_BingWang_mapstatetoprops, ERPC_ThreeDApp_BingWang_dispatchtorprops)(ERPC_ThreeDApp_BingWang);
}

gNeedCallOnErpControlInit_arr.push(InitThreedApp_BingWang);

function popBingWangSelector(completeCallBack) {
    ...
    gDataCache.set('M_Page_BingWangSelentryParam', popPage_1entryParam);
    init_M_Page_ComSel();
    popPersistentPage('M_Page_BingWangSel', function () {
        return React.createElement(VisibleCM_Page_BingWangSel, { key: 'M_Page_BingWangSel' });
    });
}

// M_Page_BingWangSel
function init_M_Page_BingWangSel(state, parentPageID) {
    var needSetState = { parentPageID: parentPageID };
    var fullParentPath = 'M_Page_BingWangSel';
    var hadState = state != null;
    if (!hadState) {
        state = store.getState();
    }
    setTimeout(function () {
        M_Page_BingWangSel_onLoad();
    }, 50);
    needSetState['M_Page_BingWangSel.parentPageID'] = parentPageID;
    needSetState['M_Page_BingWangSel._magicobj'] = {};
    if (hadState) {
...
function M_Page_BingWangSel_onLoad() {}
function M_iframe_BingWangSel_onReceiveMsg(msgtype, data, fullPath) {
    var state = store.getState();
    if (msgtype == '并网已选择') {
        closePage2('M_Page_BingWangSel', state);
        var closePage_0_callback = getPageEntryParam('M_Page_BingWangSel', 'completeCallBack');
...
var CM_Page_BingWangSel = function (_React$PureComponent2) {
    _inherits(CM_Page_BingWangSel, _React$PureComponent2);

    function CM_Page_BingWangSel(props) {
        _classCallCheck(this, CM_Page_BingWangSel);

        var _this5 = _possibleConstructorReturn(this, (CM_Page_BingWangSel.__proto__ || Object.getPrototypeOf(CM_Page_BingWangSel)).call(this, props));

        _this5.id = 'M_Page_BingWangSel';
        ERPC_Page(_this5);
        return _this5;
    }

    _createClass(CM_Page_BingWangSel, [{
        key: 'render',
        value: function render() {
            var retElem = null;
        ...
        value: function renderContent() {
            ...
            retElem = React.createElement(
                ...
                React.createElement(VisibleERPC_IFrame, { className: 'flex-grow-1 flex-shrink-1 d-flex flex-column ', proj: 'mdgjy', flowCode: '295', pageType: 'auto', onMessageFun: M_iframe_BingWangSel_onReceiveMsg, id: 'M_iframe_BingWangSel', parentPath: 'M_Page_BingWangSel' }),
                this.renderSidePage()
            );
            return retElem;
        }
    ...
    return CM_Page_BingWangSel;
}(React.PureComponent);

function CM_Page_BingWangSel_mapstatetoprops(state, ownprops) {
    var retProps = {};
    var pageState = getStateByPath(state, 'M_Page_BingWangSel', {});
    retProps['sidepages_arr'] = pageState.sidepages_arr;
    retProps['parentPageID'] = pageState.parentPageID;
    return retProps;
}
function CM_Page_BingWangSel_disptchtoprops(dispatch, ownprops) {
    var retDispath = {};
    return retDispath;
}
var VisibleCM_Page_BingWangSel = ReactRedux.connect(CM_Page_BingWangSel_mapstatetoprops, CM_Page_BingWangSel_disptchtoprops)(CM_Page_BingWangSel);