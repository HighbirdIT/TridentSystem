...
function closePage2(pid, state) {
    ...
    if (pageState.parentPageID) {
        ...
        if (parentPageState) {
            var newArr = parentPageState.sidepages_arr.filter(function (x) {
                return x.key != pid;
            });
            setTimeout(function () {
...
function getPagePath(name, stepcode, stepdata, pagetype) {
    if (pagetype == null || pagetype == 'auto') {
        pagetype = isMobile ? 'mb' : 'pc';
    }
    var targetPath = '/erppage/' + pagetype + '/' + name;
    ...
    _createClass(ERPC_DropDown_PopPanel, [{
        ...
        value: function render() {
            ...
            if (this.state.fetchingErr) {
                ...
                if (!multiselect && options_arr.length > 5) {
...
var ERPC_DropDown = function (_React$PureComponent4) {
    ...
    _createClass(ERPC_DropDown, [{
        ...
        key: 'selectItem',
        value: function selectItem(theOptionItem, autoClose, notTrigerChanged) {
            var value = null;
            var text = null;
            var values_arr = [];
            ...
            this.confirmChanged(text, value, theOptionItem, values_arr, notTrigerChanged);
        }
    }, {
        key: 'confirmChanged',
        value: function confirmChanged(text, value, theOptionItem, values_arr, notTrigerChanged) {
            ...
            if (!notTrigerChanged && typeof this.props.onchanged === 'function') {
        ...
        value: function getPopItemStyle() {
            ...
            var temElem = rootDiv.offsetParent;
            var offsetParent = null;
            var translateX = 0;
            var translateY = 0;
            var reg = /\d+\.?\d+/;
            var usePosition = '';
            var bFoundFffsetParent = false;
            while (temElem) {
                var hadTranslate = false;
                var $_temElem = $(temElem);
                var theTransFrom = $_temElem.css('transform');
                if (theTransFrom != 'none') {
                ...
                if (!bFoundFffsetParent) {
                    ...
                    if (offsetParent && elemPosition != usePosition) {
                        bFoundFffsetParent = true;
                    } else {
                        switch (elemPosition) {
                            case 'absolute':
                                {
                                    if (hadTranslate) {
                                        usePosition = elemPosition;
                                        offsetParent = temElem;
                                    }
                                    break;
                                }
                            case 'fixed':
            ...
            var offsetRect = {
                top: rootRect.top - offsetParentRect.top,
                left: rootRect.left - offsetParentRect.left,
            ...
            var rlt = {
                width: useWidth + 'px',
                left: (offsetRect.left + useWidth < windowWidth ? offsetRect.left : windowWidth - useWidth - 10) - translateX + 'px'
            };
            if (bottomSpace > topSpace) {
                rlt.top = offsetRect.top + offsetRect.height - translateY + 'px';
                rlt.maxHeight = bottomSpace - 20 + 'px';
            } else {
                rlt.bottom = offsetParentRect.height - offsetRect.top - translateY + 'px';
                ...
                if (!IsEmptyString(selectedVal)) {
                    if (IsEmptyString(selectedText)) {
                        if (selectedVal == this.props.starval) {
                            ...
                            setTimeout(function () {
                                self.selectItem(null, null, true);
                            }, 50);
                        } else {
                            if (this.props.optionsData.options_arr == null) {
                                ...
                                if (multiselect) {
                                    ...
                                    setTimeout(function () {
                                        self.selectItem(theOptionItem, null, true);
                                    }, 50);
                                }
                            }
                        }
                        ...
                        if (selectedVal != this.props.starval) {
                            if (multiselect) {
                                ...
                                if (selectedItems_arr) {
                                    ...
                                    setTimeout(function () {
                                        self.selectItem(null, false, true);
                                    }, 50);
                                }
                            } else {
                                ...
                                if (selectedOptionItem) {
                                    if (selectedOptionItem.text != selectedText) {
                                        setTimeout(function () {
                                            self.selectItem(selectedOptionItem, false, true);
                                        }, 50);
                                    }
                                } else {
                                    setTimeout(function () {
...
var selectERPC_DropDown_multiValues = function selectERPC_DropDown_multiValues(xmlstr) {
    return ERPXMLToolKit.extractColumn(xmlstr, 1).map(function (x) {
        return x.toString();
    });
};
    ...
    _createClass(ERPC_Label, [{
        ...
        value: function render() {
            ...
            if (this.props.fetching) {
                ...
                contentElem = FormatStringValue(this.props.text, this.props.type, this.props.precision, this.props.dotsplit);
...
var ERPC_Img = function (_React$PureComponent9) {
    ...
    _createClass(ERPC_Img, [{
        key: 'dataURIToBlob',
        value: function dataURIToBlob(base64) {
        ...
        value: function mouseDownHandler(ev) {
            if (this.props.onMouseDown) {
                ...
                var isBase64 = this.props.src.indexOf("base64,") != -1;
                if (isBase64) {
                    if (isMobile) {
                        ...
                        var p1 = this.props.src.indexOf('/');
                        var p2 = this.props.src.indexOf(';');
...
function ERPC_PageForm(target) {
    ...
    target.componentWillUnmount = ERPC_PageForm_WillUnmount.bind(target);
    target.componentWillMount = ERPC_PageForm_Willmount.bind(target);
}

function ERPC_PageForm_WillUnmount() {
    var formPath = this.props.fullPath;
    var scrollerElem = document.getElementById(formPath + 'scroller');
    ERPC_SaveScrollerState(formPath, scrollerElem);
}

function ERPC_PageForm_Willmount() {
    var formPath = this.props.fullPath;
    ERPC_LoadScrollerState(formPath, formPath + 'scroller');
}
...
function ERPC_Page_RenderSidePage() {
    ...
    if (sidepages_arr && sidepages_arr.length > 0) {
        return sidepages_arr.map(function (sidePage) {
            ...
            if (sidePage.closing && sidePage.closeTimer == null) {
                var closeAnim = 'anim_disappear';
                sidePage.style = Object.assign({}, sidePage.style, { animationName: closeAnim });
                var self = _this17;
                sidePage.closeTimer = setTimeout(function () {
                    var nowState = store.getState();
                    var ppPageState = getStateByPath(nowState, self.id);
                    var newArr = ppPageState.sidepages_arr.filter(function (x) {
                        return x != sidePage;
                    });
                    store.dispatch(makeAction_setStateByPath(newArr, self.id + '.sidepages_arr'));
                    if (sidePage.onClose) {
                        sidePage.onClose();
                    } else {
                        var callBack = getPageEntryParam(sidePage.key, 'callBack');
                        if (callBack) {
                            callBack({});
                        }
                    }
                }, 500);
                console.log("sidePage.close");
            }

            return React.createElement(
                'div',
                { key: sidePage.key, className: 'poppage_side bg-light', type: sidePage.type, style: sidePage.style },
                sidePage.elem,
                React.createElement(
                    'div',
                    { className: 'bg-primary border-dark closeSideBar', pkey: sidePage.key, type: sidePage.type, onClick: _this17.clickSidePageBar },
                    React.createElement('i', { className: 'fa text-light ' + iconType })
                )
            );
        });
    }
...
function ERPC_Page_RemoveSidePage(pageKey) {
    ...
    var pageElem = ppPageState.sidepages_arr.find(function (x) {
        return x.key == pageKey;
    });
    if (!pageElem.closing) {
        pageElem.closing = true;
        var newArr = ppPageState.sidepages_arr.concat();
...
function ERPC_GridForm(target) {
    ...
    target.leftTableBodyScroll = ERPC_GridForm_LeftTableBodyScroll.bind(target);
    target.rightTableBodyScroll = ERPC_GridForm_RightTableBodyScroll.bind(target);
    target.tableBodyScroll = ERPC_GridForm_TableBodyScroll.bind(target);

    target.componentWillUnmount = ERPC_GridForm_WillUnmount.bind(target);
    target.componentWillMount = ERPC_GridForm_Willmount.bind(target);
}

function ERPC_SaveScrollerState(key, scrollerElem) {
    if (scrollerElem) {
        gDataCache.set(key + 'scrollsetting', { left: scrollerElem.scrollLeft, top: scrollerElem.scrollTop });
    }
}

function ERPC_LoadScrollerState(key, elemKey) {
    var scrollSetting = gDataCache.get(key + 'scrollsetting');
    if (scrollSetting) {
        setTimeout(function () {
            var scrollerElem = document.getElementById(elemKey);
            if (scrollerElem) {
                scrollerElem.scrollTop = scrollSetting.top;
                scrollerElem.scrollLeft = scrollSetting.left;
            }
        }, 200);
    }
}

function ERPC_GridForm_Willmount() {
    var formPath = this.props.fullPath;
    var scrollSetting = gDataCache.get(formPath + 'scrollsetting');
    if (scrollSetting) {
        setTimeout(function () {
            var scrollerElem = document.getElementById(formPath + 'scroller');
            if (scrollerElem) {
    ...
    console.log('哈哈哈哈!!!!!!!!' + this.id);
}

function ERPC_GridForm_WillUnmount() {
    var formPath = this.props.fullPath;
    var scrollerElem = document.getElementById(formPath + 'scroller');
    ERPC_SaveScrollerState(formPath, scrollerElem);
}
...
function ERPC_GridForm_AntiSelect() {
    ...
    this.props.records_arr.forEach(function (r) {
        if (_this19.props.selectedValues_arr.indexOf(r._key) == -1) {
...
function ERPC_GridForm_GetRowPath(rowkey) {
    return MakePath(this.props.fullParentPath, this.props.id, 'row_' + rowkey);
}
...
function ERPC_GridForm_LeftTableBodyScroll(ev) {
    if (this.preScroll == 'right') {
        var passSec = (new Date().getTime() - this.preScrollTime) / 1000;
        if (passSec < 0.2) return;
    }
    this.preScroll = 'left';
    this.preScrollTime = new Date().getTime();
    document.getElementById(this.props.fullPath + 'rightScroller').scrollTop = ev.target.scrollTop;
}

function ERPC_GridForm_RightTableBodyScroll(ev) {
    if (this.preScroll == 'left') {
        var passSec = (new Date().getTime() - this.preScrollTime) / 1000;
        if (passSec < 0.2) return;
    }
    this.preScroll = 'right';
    this.preScrollTime = new Date().getTime();
    document.getElementById(this.props.fullPath + 'rightTableheader').scrollLeft = ev.target.scrollLeft;

    var targetRect = ev.target.getBoundingClientRect();
    if (ev.target.scrollWidth - targetRect.width > 1) {
        document.getElementById(this.props.fullPath + 'leftScrollerBottomspan').style.height = targetRect.height - ev.target.clientHeight + 'px';
    } else {
        document.getElementById(this.props.fullPath + 'leftScrollerBottomspan').style.height = '0px';
    }
    document.getElementById(this.props.fullPath + 'leftScroller').scrollTop = ev.target.scrollTop;
}

function ERPC_GridForm_TableBodyScroll(ev) {
...
var ERPC_GridForm_BtnCol = function (_React$PureComponent13) {
    ...
    function ERPC_GridForm_BtnCol(props) {
        ...
        var _this20 = _possibleConstructorReturn(this, (ERPC_GridForm_BtnCol.__proto__ || Object.getPrototypeOf(ERPC_GridForm_BtnCol)).call(this, props));
        ...
        value: function render() {
            ...
            return React.createElement(
                ...
                this.props.form.btns.map(function (btn) {
                    return React.createElement(
                        'button',
                        { key: btn.key, onClick: _this21.clickHandler, 'd-key': btn.key, className: 'btn btn-sm btn-dark', type: 'button' },
                        btn.content
                    );
                })
            );
...
var ERPC_GridSelectableRow = function (_React$PureComponent14) {
    ...
    function ERPC_GridSelectableRow(props) {
        ...
        var _this22 = _possibleConstructorReturn(this, (ERPC_GridSelectableRow.__proto__ || Object.getPrototypeOf(ERPC_GridSelectableRow)).call(this, props));

        _this22.clickHandler = _this22.clickHandler.bind(_this22);
        _this22.onMouseDown = _this22.onMouseDown.bind(_this22);
        _this22.onMouseUp = _this22.onMouseUp.bind(_this22);
        _this22.onMouseOver = _this22.onMouseOver.bind(_this22);
        _this22.clickRowHandler = _this22.clickRowHandler.bind(_this22);
        return _this22;
    }
    ...
    _createClass(ERPC_GridSelectableRow, [{
        ...
        value: function clickRowHandler(ev) {
            if (this.props.hideSelector || ev.target.tagName == 'TD' || ev.target.tagName == 'TR') {
                this.clickHandler();
            }
        }
    }, {
        ...
        value: function render() {
            ...
            return React.createElement(
                'tr',
                { onClick: this.props.rowClickable ? this.clickRowHandler : null, className: checked ? 'bg-warning' : null },
                selectElem ? React.createElement(
                    'td',
...
function ERPC_Accordion_Rebind() {
    ...
    if (typeof this.props.onUnCollapse === 'function') {
        setTimeout(function () {
            _this23.props.onUnCollapse(_this23.props.fullPath);
        }, 200);
    }
}
...
var CToastManger = function (_React$PureComponent16) {
    ...
    function CToastManger(props) {
        ...
        var _this25 = _possibleConstructorReturn(this, (CToastManger.__proto__ || Object.getPrototypeOf(CToastManger)).call(this, props));
...
var CMessageBox = function (_React$PureComponent17) {
    ...
    function CMessageBox(props) {
        ...
        var _this26 = _possibleConstructorReturn(this, (CMessageBox.__proto__ || Object.getPrototypeOf(CMessageBox)).call(this, props));

        autoBind(_this26);

        _this26.props.msgItem.changedAct = _this26.msgItemChanedHandler;
        return _this26;
    }
            ...
            if (type == EMessageBoxType.Loading) {
                ...
                if (msgItem.btns == null) {
                    ...
                    btnsElem = msgItem.btns.map(function (btn) {
                        return React.createElement(
                            'button',
                            { onClick: _this27.clickBtnHandler, key: btn.label, 'd-type': btn.key, type: 'button', className: btn.class == null ? 'btn btn-light' : btn.class },
                            btn.label
                        );
                    });
                }
...
var CMessageBoxManger = function (_React$PureComponent18) {
    ...
    function CMessageBoxManger(props) {
        ...
        var _this28 = _possibleConstructorReturn(this, (CMessageBoxManger.__proto__ || Object.getPrototypeOf(CMessageBoxManger)).call(this, props));
        ...
        value: function render() {
            ...
            return React.createElement(
                ...
                visibleMsg_arr.map(function (msg, index) {
                    return React.createElement(CMessageBox, { key: 1, msgItem: msg, manager: _this29 });
                })
            );
        }
    }]);
...
var ERPC_FrameSet = function (_React$PureComponent19) {
    ...
    function ERPC_FrameSet(props) {
        ...
        var _this30 = _possibleConstructorReturn(this, (ERPC_FrameSet.__proto__ || Object.getPrototypeOf(ERPC_FrameSet)).call(this, props));

        _this30.headerItemStyle = { minWidth: '6em' };
        _this30.clickCloseHandler = _this30.clickCloseHandler.bind(_this30);
        _this30.clickHeaderHandler = _this30.clickHeaderHandler.bind(_this30);
        return _this30;
    }
    ...
    _createClass(ERPC_FrameSet, [{
        ...
        value: function clickCloseHandler() {
            ...
            this.props.items_arr.forEach(function (item, index) {
                if (item.key != _this31.props.selectedKey) {
            ...
            if (items_arr != null) {
                ...
                itemsElem_arr = items_arr.map(function (item) {
                    ...
                    var btnElem = React.createElement(
                        'button',
                        { 'd-key': item.key, onClick: _this32.clickHeaderHandler, key: item.key, type: 'button', className: 'btn btn-sm rounded-0 btn-' + (isSelected ? 'light' : 'secondary'), style: _this32.headerItemStyle },
                        item.title
                    );
                    if (isSelected) {
                        selectedItem = item;
                        if (item.closeable != false) {
                            return [btnElem, React.createElement('button', { onClick: _this32.clickCloseHandler, key: 'close', type: 'button', className: 'btn btn-sm btn-light fa fa-close text-secondary rounded-0' })];
                        }
                    }
                    return btnElem;
                });
...
var ERPC_TopLevelFrame = function (_React$PureComponent20) {
    ...
    function ERPC_TopLevelFrame(props) {
        ...
        var _this33 = _possibleConstructorReturn(this, (ERPC_TopLevelFrame.__proto__ || Object.getPrototypeOf(ERPC_TopLevelFrame)).call(this, props));
        ...
        _this33.onloadHandler = _this33.onloadHandler.bind(_this33);
        _this33.onErrorHandler = _this33.onErrorHandler.bind(_this33);
        _this33.sendMessage = _this33.sendMessage.bind(_this33);
        _this33.push = _this33.push.bind(_this33);
        _this33.pop = _this33.pop.bind(_this33);
        _this33.iframeRef = React.createRef();
        return _this33;
    }
...
var ERPC_IFrame = function (_React$PureComponent21) {
    ...
    function ERPC_IFrame(props) {
        ...
        var _this34 = _possibleConstructorReturn(this, (ERPC_IFrame.__proto__ || Object.getPrototypeOf(ERPC_IFrame)).call(this, props));

        _this34.onloadHandler = _this34.onloadHandler.bind(_this34);
        _this34.onErrorHandler = _this34.onErrorHandler.bind(_this34);
        _this34.sendMessage = _this34.sendMessage.bind(_this34);
        _this34.trySendMsg = _this34.trySendMsg.bind(_this34);
        _this34.frameRef = React.createRef();
        _this34.tryIntval = null;
        _this34.tryCount = 0;
        return _this34;
    }
    ...
    _createClass(ERPC_IFrame, [{
        ...
        value: function trySendMsg(needDelay) {
            ...
            if (!sendOK) {
                ...
                if (needDelay) {
                    setTimeout(function () {
                        store.dispatch(makeAction_setStateByPath(null, _this35.props.fullPath + '.msg'));
                    }, 20);
                } else {
...
function ERPC_IFrame_mapstatetoprops(state, ownprops) {
    ...
    if (proj != null) {
        ...
        var pageType = ctlState.pageType ? ctlState.pageType : ownprops.pageType;
        usesrc = getPagePath(proj, flowCode, flowData, pageType);
    }
...
var ERPXMLToolKit = {
    ...
    extractAllColumn: function extractAllColumn(xmldoc) {
        var rlt = [];
        if (typeof xmldoc === 'string') {
            if (xmldoc.length == 0) {
                return rlt;
        ...
        var rootNode = xmldoc.childNodes[0];
        var fNumAttr = rootNode.attributes['fNum'];
        if (fNumAttr == null) {
            return rlt;
        }
        var colNum = parseFloat(fNumAttr.value);
        if (isNaN(colNum)) {
            return rlt;
        }
        rootNode.childNodes.forEach(function (node) {
            ...
            for (var ci = 1; ci <= colNum; ++ci) {
                var val = node.attributes['f' + ci];
                rowItem['f' + ci] = val == null || IsEmptyString(val.value) ? '' : !isNaN(val.value) ? parseFloat(val.value) : val.value;
            }
        });
        return rlt;
    },
...
function GenFormJSONData(formState, getRowItemFun, formPath, headers, state) {
    var records_arr = formState.records_arr;
    var i;
    var rlt = {
        json: null,
    ...
    for (var ri = 0; ri < records_arr.length; ++ri) {
        var record = records_arr[ri];
        var rowItem = getRowItemFun(formPath, record, state);
        if (rowItem != null) {
...
var ERPCFormSetting = function () {
    ...
    _createClass(ERPCFormSetting, [{
        ...
        value: function clear() {
            this.filters_arr.forEach(function (filter) {
                if (filter.reset()) {
                    if (filter.header) {
                        filter.header.reRender();
                    }
                }
            });
            this.filters_arr = [];
