...
var TreeControl = function (_React$PureComponent) {
    ...
    function TreeControl(props) {
        ...
        _this.keyIndex = -1;
        _this.selectRegionScrollDivRef = React.createRef();
        return _this;
    }
    ...
    _createClass(TreeControl, [{
        ...
        value: function keyPresshandler(ev) {
            ...
            if (ev.code == 'ArrowDown' || ev.code == 'ArrowUp') {
                ev.preventDefault();
                var selectedLeaf = this.state.selected_arr.length == 0 ? null : this.state.allLeaf_arr.find(function (leafData) {
                    return leafData.value == _this2.state.selected_arr[0];
                });
                ...
                if (selectedLeaf) {
                    if (this.scrolltopInt) {
                        clearInterval(this.scrolltopInt);
                        this.scrolltopInt = null;
                        //console.log('clear');
                    }
                    //console.log(selectedLeaf);
                    setTimeout(function () {
                        //console.log(selectedLeaf);
                        var leafDiv = document.getElementById(selectedLeaf.value + '_div');
                        var scrollDiv = _this2.selectRegionScrollDivRef.current;
                        if (leafDiv) {
                            var clientRect = leafDiv.getBoundingClientRect(); // 返回元素的边框矩形，相对于浏览器左上角的坐标
                            var targetScrollTop;
                            if (clientRect.top < 0) {
                                targetScrollTop = scrollDiv.scrollTop + clientRect.top;
                                //console.log(scrollDiv.scrollTop);
                            } else if (clientRect.bottom > scrollDiv.clientHeight) {
                                targetScrollTop = scrollDiv.scrollTop + clientRect.bottom - scrollDiv.clientHeight;
                                //console.log(scrollDiv.scrollTop);
                            }

                            if (targetScrollTop != null) {
                                var self = _this2;
                                self.scrolltopInt = setInterval(function () {
                                    var dis = targetScrollTop - scrollDiv.scrollTop;
                                    var step = Math.round(dis * 0.1);
                                    if (step == 0 || Math.abs(dis) <= Math.abs(step)) {
                                        scrollDiv.scrollTop = targetScrollTop;
                                        clearInterval(self.scrolltopInt);
                                        self.scrolltopInt = null;
                                        //console.log('hit');
                                    } else {
                                        scrollDiv.scrollTop += step;
        ...
        value: function isNodeContain(nodeData, keywrod) {
            ...
            return nodeData.child.find(function (childData) {
                return _this3.isNodeContain(childData, keywrod);
            });
        }
    }, {
        key: 'renderNode',
        value: function renderNode(nodeData, keyword, renderData) {
            var _this4 = this;
            ...
            */
            ...
            if (nodeData.isleaf) {
                ...
                return React.createElement(
                    'div',
                    { id: nodeData.value + '_div', key: nodeData.value, onClick: this.clickLeafNodeHandler, className: 'list-group-item list-group-item-action ' + (this.state.selected_arr.indexOf(nodeData.value) >= 0 ? 'active' : ''), value: nodeData.value },
                    nodeData.text
                );
            }
            ...
            return React.createElement(
                ...
                React.createElement(
                    'div',
                    { className: 'collapse ' + (keyword || nodeData.unCollapsed || true ? ' show' : ''), id: nodeData.name + '_group' },
                    nodeData.child.map(function (childNode) {
                        return _this4.renderNode(childNode, keyword, renderData);
                    })
                )
            );
        }
        ...
        value: function hideNodeAndChild(theNodeData) {
            ...
            if (theNodeData.child) {
                theNodeData.child.forEach(function (childData) {
                    _this5.hideNodeAndChild(childData);
                });
            }
        }
    }, {
        ...
        value: function showNodeAndChild(theNodeData) {
            ...
            if (theNodeData.child) {
                theNodeData.child.forEach(function (childData) {
                    _this6.showNodeAndChild(childData);
                });
            }
        }
    }, {
        key: 'setNodeUnCollapse',
        value: function setNodeUnCollapse(theNodeData, value) {
            theNodeData.unCollapsed = value;
        }
    }, {
        key: 'ageInputOnChangedHanlder',
        value: function ageInputOnChangedHanlder(ev) {
            // 1、通过ev.target 找到leaftvalue attribute
            // 2、通过leaftvalue 找到leafData
            // 3、通过找到leafData设置age
        }
    }, {
        key: 'render',
        value: function render() {
            ...
            if (keyword) {
                ...
                this.state.data.forEach(function (nodeData) {
                    _this7.hideNodeAndChild(nodeData);
                });
                ...
                this.state.data.forEach(function (nodeData) {
                    _this7.showNodeAndChild(nodeData);
                });
                if (this.state.arrowControled) {
                    var selectedLeaf = this.state.allLeaf_arr.find(function (leafData) {
                        return leafData.value == _this7.state.selected_arr[0];
                    });
                    if (selectedLeaf != null) {
            ...
            var selectedLeaf_arr = this.state.allLeaf_arr.filter(function (leaf) {
                if (_this7.state.selected_arr.indexOf(leaf.value) >= 0) {
                    return true;
                }
            });
            ...
            return React.createElement(
                'div',
                { id: 'treeRoot', className: 'list-group flex-grow-1 flex-shrink-1 bg-danger', style: { height: '50%' } },
                React.createElement(
                    'div',
                ...
                React.createElement(
                    ...
                    React.createElement(
                        'div',
                        { className: 'flex-grow-0 flex-shrink-0 d-flex flex-column', ref: this.selectRegionScrollDivRef, style: { overflow: 'auto' } },
                        this.state.data.map(function (item) {
                            return _this7.renderNode(item, keyword, renderData);
                        })
                    ),
                    React.createElement(
                        'div',
                        ...
                        React.createElement(
                            ...
                            React.createElement(
                                'div',
                                { className: 'col-4 border bg-dark text-light' },
                                '\u59D3\u540D'
                            ),
                            React.createElement(
                                'div',
                        ...
                        selectedLeaf_arr.map(function (leaf) {
                            return React.createElement(
                                'div',
                                { className: 'd-flex', leaftvalue: leaf.value },
                                React.createElement(
                                    'div',
                                    { className: 'col-4 border bg-light text-primary' },
                                    leaf.text
                                ),
                                React.createElement(
                                    'div',
                                ...
                                React.createElement(
                                    ...
                                    React.createElement('input', { className: 'w-100 h-100' })
                                )
                            );
                        })
                    )
...
var MyApp = function (_React$PureComponent2) {
    ...
    function MyApp(props) {
        ...
        var _this8 = _possibleConstructorReturn(this, (MyApp.__proto__ || Object.getPrototypeOf(MyApp)).call(this, props));
    ...
    _createClass(MyApp, [{
        ...
        value: function render() {
            ...
            return React.createElement(TreeControl, { multiselect: true });
