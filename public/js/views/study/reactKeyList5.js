    ...
    _createClass(TreeControl, [{
        ...
        value: function keyPresshandler(ev) {
            ...
            if (ev.code == 'ArrowDown') {
                console.log('向下移动');
                ++this.keyIndex;
                var theLeaf = this.state.allLeaf_arr[this.keyIndex];
                var value = theLeaf.value;
                this.showNodeAndChild(theLeaf);
                this.setState({
                    selected_arr: [value],
                    arrowControled: true
                });
        ...
        value: function renderNode(nodeData, keyword) {
            ...
            */
            ...
            return React.createElement(
                ...
                React.createElement(
                    'div',
                    { className: 'collapse ' + (keyword || nodeData.unCollapsed ? ' show' : ''), id: nodeData.name + '_group' },
            ...
            if (keyword) {
                ...
                if (this.state.arrowControled) {
                    var selectedLeaf = this.state.allLeaf_arr.find(function (leafData) {
                        return leafData.value == _this6.state.selected_arr[0];
                    });
