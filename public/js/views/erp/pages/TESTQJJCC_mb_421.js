...
function button_3_onclick() {
	var state = store.getState();
	var scriptBP_32_msg = null;
	var callback_final = function callback_final(state, data, err) {
		if (err) {
			if (scriptBP_32_msg) {
				scriptBP_32_msg.setData(err.info, EMessageBoxType.Error, '申请请假');
			} else {
				SendToast(err.info, EToastType.Error);
			}
			return;
		...
		if (scriptBP_32_msg) {
			scriptBP_32_msg.fireClose();
		}
		SendToast('执行成功');
	};
	var fetchid = Math.round(Math.random() * 999999);
	fetchTracer['button_3_onclick'] = fetchid;
	scriptBP_32_msg = PopMessageBox('', EMessageBoxType.Loading, '申请请假');;
	var bundle_querysql_0 = {};
	setTimeout(function () {
		if (fetchTracer['button_3_onclick'] != fetchid) return;
		store.dispatch(fetchJsonPost(appServerUrl, { bundle: bundle_querysql_0, action: 'scriptBP_32_querysql_0' }, makeFTD_Callback(function (state, data_querysql_0, error_querysql_0) {
			if (error_querysql_0) {
				return callback_final(state, null, error_querysql_0);
			}
			...
			for (index_querysql_0 = 0; index_querysql_0 < data_querysql_0.length; ++index_querysql_0) {
				...
				var querysql_0_员工登记姓名代码 = row_querysql_0.员工登记姓名代码;
				var querysql_0_员工在职种类代码 = row_querysql_0.员工在职种类代码;
				var querysql_0_员工工时状态代码 = row_querysql_0.员工工时状态代码;
				if (querysql_0_员工登记姓名代码 == g_envVar.userid && querysql_0_员工在职种类代码 == 1 && querysql_0_员工工时状态代码 == 1) {
					var ret = callback_final(state, null, { info: '工人不可以使用钉钉请假，需要向班组长请假' });
					return ret == null ? state : ret;
				} else {
					setTimeout(function () {
...
function M_Dropdown_7_defaultvalue_get(state, bundle) {
	...
	var fetchid = Math.round(Math.random() * 999999);
	fetchTracer['M_Dropdown_7_defaultvalue_get'] = fetchid;
	state = setManyStateByPath(state, 'M_Page_5.M_Form_7.M_Dropdown_7', { fetching: true, fetchingErr: null });
	var bundle_querysql_0 = {
		员工代码: M_Dropdown_6_value
	};
	setTimeout(function () {
		if (fetchTracer['M_Dropdown_7_defaultvalue_get'] != fetchid) return;
...
function M_LC_55_isdisplay_get(state, bundle) {
	var M_Form_8_nowRecord = bundle != null && bundle.M_Form_8_nowRecord != null ? bundle.M_Form_8_nowRecord : getStateByPath(state, 'M_Page_6.M_Form_8.nowRecord');
	var validErr;
	var hadValidErr = false;
	var validErrState = {};
	var callback_final = function callback_final(state, data, err) {
...
function button_10_onclick() {
	...
	var M_Form_8_nowRecord = getStateByPath(M_Form_8_state, 'nowRecord');
	var validErr;
	var hadValidErr = false;
	var validErrState = {};
...
function M_Dropdown_6_value_changed(state, newValue, oldValue, path, visited, delayActs) {
	var needSetState = {};
						...
						retElem = React.createElement(
							...
							React.createElement(
								...
								React.createElement(
									...
									React.createElement(VisibleERPC_DropDown, { id: "M_Dropdown_7", parentPath: "M_Page_5.M_Form_7", pullDataSource: pull_M_Dropdown_7, textAttrName: "\u8003\u52E4\u767B\u8BB0\u5730\u70B9", valueAttrName: "\u8003\u52E4\u767B\u8BB0\u5730\u70B9\u4EE3\u7801", label: "\u5F53\u524D\u5DE5\u4F5C\u5730\u70B9" })
								),
								React.createElement(
									VisibleERPC_LabeledControl,
