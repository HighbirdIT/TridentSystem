...
function active_M_Page_0(state) {
	...
	needSetState['M_Page_0.M_Text_0.value'] = null;
...
function button_0_onclick() {
	...
	setTimeout(function () {
		...
		store.dispatch(fetchJsonPost(appServerUrl, { bundle: bundle_querysql_0, action: '_query_TTest' }, makeFTD_Callback(function (state, data_querysql_0, error_querysql_0) {
			if (error_querysql_0) {
				return callback_final(state, null, error_querysql_0);
			}
			var index_querysql_0;
			for (index_querysql_0 = 0; index_querysql_0 < data_querysql_0.length; ++index_querysql_0) {
				...
				sum = sum + querysql_0_isd + querysql_0_记录代码;
			}
			setTimeout(function () {
				store.dispatch(makeAction_setStateByPath(sum, 'M_Page_0.M_Text_0.value'));
			}, 50);
		...
		value: function renderContent() {
			...
			retElem = React.createElement(
				...
				React.createElement(
					...
					React.createElement(VisibleERPC_Text, { id: 'M_Text_0', parentPath: 'M_Page_0', type: 'string', linetype: 'single' })
				),
				React.createElement(
					'button',
