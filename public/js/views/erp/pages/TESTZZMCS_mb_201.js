...
function active_M_Page_2(state) {
	...
	//M_Text_4_defaultvalue_get(state);
...
function M_Text_4_defaultvalue_get(state, bundle) {
	var bundle = {
		员工代码: getStateByPath(state, 'M_Page_2.M_Dropdown_0.value'),
		假期种类代码: getStateByPath(state, 'M_Page_2.M_Dropdown_1.value')
	};
	var callbackFun_1 = function callbackFun_1(state, data) {
		return setStateByPath(state, 'M_Page_2.M_Text_4.value', data);
	};
	setTimeout(function () {
		//store.dispatch(fetchJsonPost(appServerUrl, {bundle:bundle,action:'getQJInfo',}, makeFTD_Prop('M_Page_2','M_Text_4','value',false), EFetchKey.FetchPropValue));
		store.dispatch(fetchJsonPost(appServerUrl, { bundle: bundle, action: 'getQJInfo' }, makeFTD_Callback(function (state, data) {
			var ret = callbackFun_1(state, data);
