...
function M_Text_2_validchecker(nowValue, comeState, comeValidErrState) {
	...
	if (validErrState.hasOwnProperty('M_Page_1.M_Text_2.invalidInfo')) {
		validErr = validErrState['M_Page_1.M_Text_2.invalidInfo'];
	} else {
		validErr = BaseIsValueValid(comeState, M_Text_2_state, M_Text_2_state, M_Text_2_value, 'date', false, 'M_Text_2', validErrState);
		validErrState['M_Page_1.M_Text_2.invalidInfo'] = validErr;
	}
	if (validErr != null) hadValidErr = true;
	if (hadValidErr) {
	...
	if (M_Text_2_value > getFormatDateString(new Date(getNowDate().setDate(getNowDate().getDate() + -3)))) {
		var ret = callback_final(state, null, { info: '只能认定三天前的考勤' });
		return ret == null ? state : ret;
	}
}
						...
						if (!this.props.canInsert && this.props.nowRecord == null) {
							...
							retElem = React.createElement(
								...
								React.createElement(
									...
									React.createElement(VisibleERPC_Label, { className: "text-danger bg-primary font-weight-bold max-height 50% erp-control ", id: "M_Label_8", parentPath: "M_Page_1.M_Form_4", type: "string" }),
