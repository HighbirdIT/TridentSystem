...
function bind_M_Form_0(retState, newIndex, oldIndex) {
	...
	if (needActiveBindPage) {
		retElem = setManyStateByPath(retState, 'M_Page_0.M_Form_0', needSetState);
		return bind_M_Form_0Page(retElem);
	}
...
function bind_M_Form_0Page(retState) {
	...
	var endRowIndex = pageIndex * rowPerPage - 1;
	for (var rowIndex = startRowIndex; rowIndex <= endRowIndex; ++rowIndex) {
		var rowRecord = records_arr[rowIndex];
		needSetState['M_Label_0.text'] = nowRecord['员工登记姓名'];
		needSetState['M_Label_1.text'] = nowRecord['员工登记性别'];
		needSetState['M_Label_2.text'] = nowRecord['身份证件号码'];
		needSetState['M_Label_3.text'] = nowRecord['身份证件地址'];
		needSetState['M_Label_4.text'] = nowRecord['出生年月日期'];
	}
