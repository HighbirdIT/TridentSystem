...
function M_Dropdown_2_defaultvalue_get(state, bundle) {
	var callback_final = function callback_final(state, data, err) {
		...
		return setManyStateByPath(state, 'M_Page_0.M_Form_0.M_Dropdown_2', needSetState);
	};
	var fetchid = Math.round(Math.random() * 999999);
	fetchTracer['M_Dropdown_2_defaultvalue_get'] = fetchid;
	state = setManyStateByPath(state, 'M_Page_0.M_Form_0.M_Dropdown_2', { fetching: true, fetchingErr: null });
	var bundle_querysql_0 = {
		员工工号: g_envVar.userid
	};
	setTimeout(function () {
		if (fetchTracer['M_Dropdown_2_defaultvalue_get'] != fetchid) return;
	...
	setTimeout(function () {
		store.dispatch(fetchJsonPost(appServerUrl, { bundle: bundle_insert_table_0, action: 'button_0_onclick_insert_table_0' }, makeFTD_Callback(function (state, data_insert_table_0, err_insert_table_0) {
			if (err_insert_table_0 == null) {
				...
				if (M_Text_1_value == getFormatDateString(getNowDate())) {
					setTimeout(function () {
						pull_M_Form_1();
					}, 50);
				}
