			...
			if (querysql_0_日期 == null) {
				...
				setTimeout(function () {
					...
					store.dispatch(fetchJsonPost(appServerUrl, { bundle: bundle_querysql_1, action: 'scriptBP_12_querysql_1' }, makeFTD_Callback(function (state, data_querysql_1, error_querysql_1) {
						...
						return querysql_1_员工在职起始;
					}, false)));
				}, 50);
			} else {
				return querysql_0_日期;
			}
		}, false)));
	}, 50);
}
		...
		value: function renderContent() {
			...
			retElem = React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						"button",
						{ className: "btn btn-primary erp-control ", id: "button_5" },
						"\u524D\u4E00\u5929"
					),
					React.createElement(VisibleERPC_Text, { className: "flex-grow-1 ", id: "M_Text_2", parentPath: "M_Page_1", type: "date" }),
					React.createElement(
						"button",
						{ className: "btn btn-primary erp-control ", id: "button_6" },
						"\u540E\u4E00\u5929"
					)
				),
