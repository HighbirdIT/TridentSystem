...
var appStateChangedAct_map = { 'M_Page_0.M_Form_0.M_Text_0.value': M_Text_0_value_changed.bind(window), 'M_Page_0.M_Form_0.M_Text_1.value': M_Text_1_value_changed.bind(window) };
var pageRouter = [];
...
function M_Label_1_textfield_get(state, bundle) {
	var M_Form_0_state = getStateByPath(state, 'M_Page_0.M_Form_0', {});
	...
	var callback_final = function callback_final(state, data, err) {
		...
		needSetState.fetching = false;
		needSetState.fetchingErr = err;
		return setManyStateByPath(state, 'M_Page_0.M_Form_0.M_Label_1', needSetState);
	};
	...
	var fetchid = Math.round(Math.random() * 999999);
	fetchTracer['M_Label_1_textfield_get'] = fetchid;
	state = setManyStateByPath(state, 'M_Page_0.M_Form_0.M_Label_1', { fetching: true, fetchingErr: null });
	var bundle_querysql_0 = {
		往来记录代码: M_Text_1_value
	};
	setTimeout(function () {
		if (fetchTracer['M_Label_1_textfield_get'] != fetchid) return;
		store.dispatch(fetchJsonPost(appServerUrl, { bundle: bundle_querysql_0, action: 'scriptBP_0_querysql_0' }, makeFTD_Callback(function (state, data_querysql_0, error_querysql_0) {
			if (error_querysql_0) {
				return callback_final(state, null, error_querysql_0);
			}
			var ret = callback_final(state, data_querysql_0, error_querysql_0);
...
function M_Text_1_value_changed(state, newValue, oldValue, path, visited, delayActs) {
	var needSetState = {};
	needSetState['M_Page_0.M_Form_0.M_Label_1.text'] = M_Label_1_textfield_get(state);
	return setManyStateByPath(state, '', needSetState);
}
						...
						retElem = React.createElement(
							...
							React.createElement(
								...
								React.createElement(
									...
									React.createElement(VisibleERPC_Label, { className: 'erp-control ', id: 'M_Label_1', parentPath: 'M_Page_0.M_Form_0', type: 'int' })
								)
							),
