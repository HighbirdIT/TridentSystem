...
function M_Dropdown_7_defaultvalue_get(state, bundle) {
	...
	setTimeout(function () {
		...
		store.dispatch(fetchJsonPost(appServerUrl, { bundle: bundle_querysql_0, action: 'querysql_0' }, makeFTD_Callback(function (state, data_querysql_0, error_querysql_0) {
			...
			var ret = callback_final(state, error_querysql_0, null);
			return ret == null ? state : ret;
		}, false)));
	}, 50);
}
...
function M_Text_18_defaultvalue_get(state, bundle) {
	var M_Form_7_state = getStateByPath(state, 'M_Page_5.M_Form_7', {});
	var M_CheckBox_0_state = getStateByPath(M_Form_7_state, 'M_CheckBox_0', {});
	var M_LC_46_state = getStateByPath(M_Form_7_state, 'M_LC_46', {});
	var M_CheckBox_0_value = bundle != null && bundle['M_CheckBox_0_value'] != null ? bundle['M_CheckBox_0_value'] : M_CheckBox_0_state.value;
...
function M_Text_20_defaultvalue_get(state, bundle) {
	var M_Form_7_state = getStateByPath(state, 'M_Page_5.M_Form_7', {});
	var M_Dropdown_6_state = getStateByPath(M_Form_7_state, 'M_Dropdown_6', {});
	var M_LC_40_state = getStateByPath(M_Form_7_state, 'M_LC_40', {});
	var M_Dropdown_6_text = bundle != null && bundle['M_Dropdown_6_text'] != null ? bundle['M_Dropdown_6_text'] : M_Dropdown_6_state.text;
	var validErr;
	var hadValidErr = false;
	var validErrState = {};
	var callback_final = function callback_final(state, data, err) {
		...
		needSetState.fetching = false;
		needSetState.fetchingErr = err;
		return setManyStateByPath(state, 'M_Page_5.M_Form_7.M_Text_20', needSetState);
	};
	validErr = BaseIsValueValid(state, M_LC_40_state, M_Dropdown_6_state, M_Dropdown_6_text, 'string', false, 'M_Dropdown_6', validErrState);
	validErrState['M_Page_5.M_Form_7.M_Dropdown_6.invalidInfo'] = validErr;
	if (validErr != null) hadValidErr = true;
	if (hadValidErr) {
		return callback_final(state, null, { info: gPreconditionInvalidInfo });
	...
	var fetchid = Math.round(Math.random() * 999999);
	fetchTracer['M_Text_20_defaultvalue_get'] = fetchid;
	state = setManyStateByPath(state, 'M_Page_5.M_Form_7.M_Text_20', { fetching: true, fetchingErr: null });
	var bundle_querysql_0 = {
		员工姓名: M_Dropdown_6_text
	};
	setTimeout(function () {
		if (fetchTracer['M_Text_20_defaultvalue_get'] != fetchid) return;
		store.dispatch(fetchJsonPost(appServerUrl, { bundle: bundle_querysql_0, action: 'querysql_0' }, makeFTD_Callback(function (state, data_querysql_0, error_querysql_0) {
			if (error_querysql_0) {
				return callback_final(state, null, error_querysql_0);
			}
			return querysql_0_常住工作地点代码;
		}, false)));
	}, 50);
}
...
function M_CheckBox_0_value_changed(state, newValue, oldValue, path, visited, delayActs) {
	...
	needSetState['M_Page_5.M_Form_7.M_Text_18.value'] = M_Text_18_defaultvalue_get(state);
	return setManyStateByPath(state, '', needSetState);
}
function M_Dropdown_6_text_changed(state, newValue, oldValue, path, visited, delayActs) {
	var needSetState = {};
	needSetState['M_Page_5.M_Form_7.M_Text_20.value'] = M_Text_20_defaultvalue_get(state);
	return setManyStateByPath(state, '', needSetState);
}
