...
function M_Text_2_validchecker(nowValue, comeState, comeValidErrState) {
	var state = store.getState();
	if (nowValue > getFormatDateString(new Date(getNowDate().setDate(getNowDate().getDate() + -3)))) {
		setTimeout(function () {
			store.dispatch(makeAction_setStateByPath('没有需要认定的考勤了', 'M_Page_1.M_Text_2.value'));
		}, 50);
	}
}
	...
	setTimeout(function () {
		...
		store.dispatch(fetchJsonPost(appServerUrl, { bundle: bundle_querysql_0, action: 'scriptBP_16_querysql_0' }, makeFTD_Callback(function (state, data_querysql_0, error_querysql_0) {
			...
			if (M_Text_2_value > data_querysql_0) {
				...
				if (data_querysql_0 > getFormatDateString(new Date(getNowDate().setDate(getNowDate().getDate() + -3)))) {
					var ret = callback_final(state, null, { info: '只能认定三天之前的考勤' });
					return ret == null ? state : ret;
				} else {
					var bundle_insert_table_0 = {
						员工登记姓名代码: g_envVar.userid,
