...
function button_2_isdisplay_get(state, bundle) {
	...
	var callback_final = function callback_final(state, data, err) {
		...
		return setManyStateByPath(state, 'M_Page_1.M_Form_4.button_2', needSetState);
	};
	if (IsEmptyString(M_Form_4_nowRecord)) {
	...
	var fetchid = Math.round(Math.random() * 999999);
	fetchTracer['button_2_isdisplay_get'] = fetchid;
	if (M_Form_4_nowRecord['是否已经认定'] == 1) {
		...
		setTimeout(function () {
			if (fetchTracer['button_2_isdisplay_get'] != fetchid) return;
			store.dispatch(fetchJsonPost(appServerUrl, { bundle: bundle_querysql_0, action: 'scriptBP_17_querysql_0' }, makeFTD_Callback(function (state, data_querysql_0, error_querysql_0) {
				if (error_querysql_0) {
					return callback_final(state, null, error_querysql_0);
				}
				return '本条记录已经在' + getFormatDateString(castDate(querysql_0_登记确认时间)) + '被' + querysql_0_登记确认人员 + '认定过，无需再进行认定';
			}, false)));
		}, 50);
	}
}
	...
	setTimeout(function () {
		...
		store.dispatch(fetchJsonPost(appServerUrl, { bundle: bundle_querysql_0, action: 'scriptBP_16_querysql_0' }, makeFTD_Callback(function (state, data_querysql_0, error_querysql_0) {
			...
			if (M_Text_2_value > data_querysql_0) {
				var ret = callback_final(state, null, { info: '所选日期不是你的考勤认定起始日期，你的考勤认定起始日期应为：' + getFormatDateString(castDate(data_querysql_0)) });
				return ret == null ? state : ret;
			} else {
				var bundle_insert_table_0 = {
					员工登记姓名代码: g_envVar.userid,
