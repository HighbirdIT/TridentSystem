...
var ControlGraphNode = function () {
    function ControlGraphNode(kernel) {
        _classCallCheck(this, ControlGraphNode);
    ...
    _createClass(ControlGraphNode, [{
        key: 'addOut',
        value: function addOut(targetNode) {
        ...
        key: 'addIn',
        value: function addIn(targetNode) {
    ...
    return ControlGraphNode;
}();

var ECtlReplyPathType = {
    SetAP_On_BPChanged: 'SetAP_On_BPChanged',
    CallFun_On_BPChanged: 'CallFun_On_BPChanged'
};

var ControlGraphPath = function ControlGraphPath(type, relyNode, berelyNode, initData) {
    _classCallCheck(this, ControlGraphPath);
...
var ControlRelyOnGraph = function () {
    function ControlRelyOnGraph() {
        _classCallCheck(this, ControlRelyOnGraph);

        this.allpath_map = {};
        this.allgraph_map = {};
    }

    _createClass(ControlRelyOnGraph, [{
        key: 'addRely_setAPOnBPChanged',
        value: function addRely_setAPOnBPChanged(relyCtl, relyPropName, berelyCtl, berelyPropName, approach) {
        ...
        key: 'addRely_CallFunOnBPChanged',
        value: function addRely_CallFunOnBPChanged(funName, berelyCtl, berelyPropName) {
    ...
    return ControlRelyOnGraph;
}();

var MobileContentCompiler = function (_ContentCompiler) {
    _inherits(MobileContentCompiler, _ContentCompiler);

    function MobileContentCompiler(projectCompiler) {
        _classCallCheck(this, MobileContentCompiler);

        var _this = _possibleConstructorReturn(this, (MobileContentCompiler.__proto__ || Object.getPrototypeOf(MobileContentCompiler)).call(this, projectCompiler));

        autoBind(_this);

        _this.conetents_arr = _this.project.content_Mobile;
        _this.compilePage = _this.compilePage.bind(_this);
        _this.blocks_map = {};
        return _this;
    }

    _createClass(MobileContentCompiler, [{
        key: 'compile',
        ...
        key: 'modifyControlTag',
        value: function modifyControlTag(ctlKernel, ctlTag) {
        ...
        key: 'compileScriptBlueprint',
        value: function compileScriptBlueprint(targetBP) {
        ...
        key: 'compilePage',
        value: function compilePage(pageKernel) {
            var clientSide = this.clientSide;
            var project = this.project;
            var logManager = project.logManager;
            ...
            if (pageMidData.needSetKernels_arr.length > 0) {
                ...
                for (ci in pageMidData.needSetKernels_arr) {
                    ...
                    if (targetKernelMidData.needSetStates_arr.length > 0) {
                        targetKernelMidData.needSetStates_arr.forEach(function (stateItem) {
        ...
        key: 'getKernelParentPath',
        value: function getKernelParentPath(theKernel) {
        ...
        key: 'compileKernel',
        value: function compileKernel(theKernel, renderBlock, renderFun) {
        ...
        key: 'genERPCTag',
        value: function genERPCTag(kernel, tagName) {
        ...
        key: 'compileTextKernel',
        value: function compileTextKernel(theKernel, renderBlock, renderFun) {
            var project = this.project;
            var logManager = project.logManager;
            ...
            if (textFieldParseRet.isScript) {
                if (defaultValParseRet.isScript) {
                    logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '为默认值、显示字段同时设置了脚本']);
                    return false;
                }
        ...
        key: 'compileCheckBoxKernel',
        value: function compileCheckBoxKernel(theKernel, renderBlock, renderFun) {
            var project = this.project;
            var logManager = project.logManager;
        ...
        key: 'compileScriptAttribute',
        value: function compileScriptAttribute(attrParseRet, theKernel, stateName, attrLabel, config) {
            var project = this.project;
            var logManager = project.logManager;
            if (attrParseRet.jsBp == null) {
                logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), attrLabel + '用到了脚本，但没有创建此脚本']);
                return false;
            }
            var belongFormKernel = theKernel.searchParentKernel(M_FormKernel_Type, true);
        ...
        key: 'compileIsdisplayAttribute',
        value: function compileIsdisplayAttribute(theKernel, ctlTag) {
        ...
        key: 'compileValidCheckerAttribute',
        value: function compileValidCheckerAttribute(theKernel) {
        ...
        key: 'compileLabeledControlKernel',
        value: function compileLabeledControlKernel(theKernel, renderBlock, renderFun) {
            var clientSide = this.clientSide;
            var layoutConfig = theKernel.getLayoutConfig();
            var project = this.project;
            var logManager = project.logManager;
            ...
            if (IsEmptyObject(label)) {
                logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '没有设置标签']);
                return false;
            }
        ...
        key: 'compileFormKernel',
        value: function compileFormKernel(theKernel, renderBlock, renderFun) {
            var project = this.project;
            var logManager = project.logManager;
            var clientSide = this.clientSide;
            ...
            if (useDS == null && isGridForm) {
                logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), 'Grid表单需要数据源']);
                return false;
            }
            ...
            formReactClass.mapStateFun.pushLine(makeLine_Assign(makeStr_DotProp(VarNames.RetProps, 'loaded'), useDS == null ? 'true' : makeStr_DotProp(VarNames.CtlState, VarNames.Records_arr) + ' != ' + null));
            if (isGridForm) {
            ...
            if (isGridForm) {
                ...
                for (ci in theKernel.children) {
                    ...
                    if (childKernel.type != M_LabeledControlKernel_Type) {
                        logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), 'Grid表单里必须全是操作控件']);
                        return false;
                    }
                    ...
                    if (isNaN(columnWidth)) {
                        logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '列' + headTextValue + '列宽设置错误']);
                        return false;
                    }
                    if (columnWidth == 0) {
                            ...
                            if (!targetKernelMidData.isSelfCare) {
                                if (formCanInsert) {
                                    ...
                                    if (visibleStyle == null) {
                                        logManager.warnEx([logManager.createBadgeItem(targetKernel.getReadableName(), targetKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '没有提供VisibleStyle']);
        ...
        key: 'compileContainerKernel',
        value: function compileContainerKernel(theKernel, renderBlock, renderFun) {
            var layoutConfig = theKernel.getLayoutConfig();
            layoutConfig.addClass('d-flex');
            layoutConfig.addClass('flex-grow-1');
            layoutConfig.addClass('flex-shrink-1');
        ...
        key: 'compileLabelKernel',
        value: function compileLabelKernel(theKernel, renderBlock, renderFun) {
            var project = this.project;
            var layoutConfig = theKernel.getLayoutConfig();
            layoutConfig.addClass('erp-control');
        ...
        key: 'compileButtonKernel',
        value: function compileButtonKernel(theKernel, renderBlock, renderFun) {
            var project = this.project;
            var layoutConfig = theKernel.getLayoutConfig();
            layoutConfig.addClass('erp-control');
        ...
        key: 'compileDropdownKernel',
        value: function compileDropdownKernel(theKernel, renderBlock, renderFun) {
            var project = this.project;
            var logManager = project.logManager;
            var clientSide = this.clientSide;
            var serverSide = this.serverSide;
            ...
            var useDS = theKernel.getAttribute(AttrNames.DataSource);
            if (useDS == null) {
                logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '需要数据源']);
                return false;
            }
            ...
            if (!useDS.containColumn(fromTextfield)) {
                logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '来源文本字段设置错误']);
                return false;
            }
            if (hadValueField && !useDS.containColumn(fromValuefield)) {
                logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '来源码值字段设置错误']);
                return false;
            }
            if (IsEmptyString(textField)) {
                logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '未设置显示字段']);
                return false;
            }
            if (hadValueField && IsEmptyString(valueField)) {
                logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '未设置码值字段']);
                return false;
            }
            if (!hadValueField && !IsEmptyString(valueField)) {
                logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '码值字段设置错误']);
                return false;
            }
            ...
            if (dataGroupAttr_arr.length > 0) {
                for (var agai in dataGroupAttr_arr) {
                    ...
                    if (groupCols_arr.indexOf(colName) != -1) {
                        logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '重复设置分层列:' + colName]);
                        return false;
                    }
                    groupCols_arr.push(colName);
                    if (colName == fromValuefield || colName == fromTextfield || canUseColumns_arr.indexOf(colName) == -1) {
                        logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '错误的分层列:' + colName]);
                        return false;
                    }
                }
            ...
            var textFieldParseRet = parseObj_CtlPropJsBind(textField, project.scriptMaster);
            if (textFieldParseRet.isScript) {
                logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '请不要为下拉框显示字段设置脚本']);
                return false;
            } else {
                var belongFormKernel = theKernel.searchParentKernel(M_FormKernel_Type, true);
                var kernelMidData = this.projectCompiler.getMidData(theKernel.id);
                ...
                if (setValueStateItem != null && setTextStateItem == null) {
                    logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '下拉框的码值字段能找到匹配但是显示字段找不到匹配']);
                    return false;
                }
        ...
        value: function compileEnd() {
            ...
            _get(MobileContentCompiler.prototype.__proto__ || Object.getPrototypeOf(MobileContentCompiler.prototype), 'compileEnd', this).call(this);
        }
    }, {
        key: 'getString',
        ...
        key: 'endKernelCompile',
        value: function endKernelCompile(kernel) {
            switch (kernel.type) {
                case M_FormKernel_Type:
        ...
        key: 'endFormKernelCompile',
        value: function endFormKernelCompile(theKernel) {
            var _this2 = this;
            ...
            if (midData.useDS) {
                ...
                if (mustSelectColumns_arr.length == 0) {
                    logManager.errorEx([logManager.createBadgeItem(theKernel.getReadableName(), theKernel, this.projectCompiler.clickKernelLogBadgeItemHandler), '没有任何列被使用']);
                } else {
                    ...
                    if (needCheckVars_arr.length > 0) {
                        ...
                        needCheckVars_arr.forEach(function (varObj) {
                            ...
                            if (varObj.propApi) {
                                _this2.ctlRelyOnGraph.addRely_CallFunOnBPChanged(theFun.name, varObj.kernel, varObj.propApi.stateName);
                            }
                            ...
                            validKernelBlock.pushLine('validErr = ' + makeStr_callFun('BaseIsValueValid', [_this2.group == EJsBluePrintFunGroup.CtlValid ? 'comeState' : VarNames.State, varObj.visibleStateVar, varObj.ctlStateVar, varObj.valueVar, singleQuotesStr(valueType), varObj.nullable.toString(), singleQuotesStr(varObj.kernel.id), validErrStateVar.name]) + ";");
