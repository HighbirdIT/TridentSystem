...
class ERPC_DropDown extends React.PureComponent {
    ...
    selectItem(theOptionItem, autoClose, notTrigerChanged) {
        var value = null;
        var text = null;
        var values_arr = [];
    ...
    confirmChanged(text, value, theOptionItem, values_arr, notTrigerChanged) {
...
const selectERPC_DropDown_multiValues = (xmlstr) => {
    return ERPXMLToolKit.extractColumn(xmlstr, 1).map(x=>x.toString());
};
    ...
    if (sidepages_arr && sidepages_arr.length > 0) {
        return sidepages_arr.map(sidePage => {
            ...
            if (sidePage.closing && sidePage.closeTimer == null) {
                ...
                var self = this;
                sidePage.closeTimer = setTimeout(() => {
                    var nowState = store.getState();
                    var ppPageState = getStateByPath(nowState, self.id);
                    var newArr = ppPageState.sidepages_arr.filter(x => { return x != sidePage; });
...
function ERPC_GridForm_ClickNewRowHandler() {
    ...
    setTimeout(()=>{
        ...
        if(newTrElem){
            ...
            if (trTop >= 30 && trTop < $(window).height() - 100) {
            }
            else{
...
class ERPC_GridSelectableRow extends React.PureComponent {
    ...
    render() {
        ...
        return <tr onClick={this.props.rowClickable ? this.clickRowHandler : null} className={checked ? 'bg-warning' : null}>
            {selectElem ? <td className='selectorTableHeader'>{selectElem}</td> : null}
            {this.props.children}
        </tr>;
    }
}
...
class ERPC_Container extends React.PureComponent {
    constructor(props) {
        super(props);
        autoBind(this);
    ...
    render() {
        ...
        */
        ...
        if(tagtype == 'div'){
            return <div className={this.props.className} style={this.props.style}>{childElem}</div>
        }
        else if(tagtype == 'span'){
            return <span className={this.props.className} style={this.props.style}>{childElem}</span>
        }
        else if(tagtype == 'h1'){
            return <h1 className={this.props.className} style={this.props.style}>{childElem}</h1>
        }
        else if(tagtype == 'h2'){
            return <h2 className={this.props.className} style={this.props.style}>{childElem}</h2>
        }
        else if(tagtype == 'h3'){
            return <h3 className={this.props.className} style={this.props.style}>{childElem}</h3>
        }
        else if(tagtype == 'h4'){
            return <h4 className={this.props.className} style={this.props.style}>{childElem}</h4>
        }
        else if(tagtype == 'h5'){
            return <h5 className={this.props.className} style={this.props.style}>{childElem}</h5>
        }
        else if(tagtype == 'h6'){
            return <h6 className={this.props.className} style={this.props.style}>{childElem}</h6>
        }
        return null;
    }
}
...
class ERPCFormSetting {
    ...
    clear(){
        this.filters_arr.forEach(filter=>{
            if(filter.reset()){
                if(filter.header){
