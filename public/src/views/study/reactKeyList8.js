...
class TreeControl extends React.PureComponent {
    constructor(props) {
        ...
        this.keyIndex = -1;
        this.selectRegionScrollDivRef = React.createRef();
    }
        ...
        if (ev.code == 'ArrowDown' || ev.code == 'ArrowUp') {
            ...
            if(selectedLeaf)
            {
                ...
                setTimeout(() => {
                    ...
                    var scrollDiv = this.selectRegionScrollDivRef.current;
                    if(leafDiv){
    ...
    renderNode(nodeData, keyword, renderData) {
        ...
        */
        ...
        if (nodeData.isleaf) {
            ...
            return (<div id={nodeData.value + '_div'} key={nodeData.value} onClick={this.clickLeafNodeHandler} className={'list-group-item list-group-item-action ' + (this.state.selected_arr.indexOf(nodeData.value) >= 0 ? 'active' : '')} value={nodeData.value}>{nodeData.text}</div>);
        }
    ...
    render() {
        ...
        var selectedLeaf_arr = this.state.allLeaf_arr.filter(
            leaf=>{
                if(this.state.selected_arr.indexOf(leaf.value) >= 0){
                    return true;
                }
            }
        );
        ...
        return (
            <div id='treeRoot' className='list-group flex-grow-1 flex-shrink-1 bg-danger' style={{height:'50%'}}>
                ...
                <div className='flex-grow-1 flex-shrink-1 d-flex'>
                    <div className='flex-grow-0 flex-shrink-0 d-flex flex-column' ref={this.selectRegionScrollDivRef} style={{overflow:'auto'}} >
                    {
                        this.state.data.map(
                            item => {
                    ...
                    <div className='flex-grow-1 flex-shrink-1 d-flex flex-column'>
                        <div className='d-flex'>
                            <div className='col-4 border bg-dark text-light'>
                                姓名
                            </div>
                            <div className='col-4 border bg-dark text-light'>
                                系统
                        ...
                        {
                            selectedLeaf_arr.map(
                                leaf=>{
                                    return (
                                        <div className='d-flex' leaftvalue={leaf.value}>
                                            <div className='col-4 border bg-light text-primary'>
                                                {leaf.text}
                                            </div>
                                            <div className='col-4 border bg-light text-primary'>
                                                暂无
...
class MyApp extends React.PureComponent {
    ...
    render() {
        ...
        return (<TreeControl multiselect={true} />);
