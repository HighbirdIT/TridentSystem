...
class C关联构件模型 {
    ...
    append(构件编号, 构件全称, 文件路径, 上传时间, 构件代码) {
...
const componentModelMat = new THREE.MeshStandardMaterial({ color: 0xFF9933, side: THREE.DoubleSide });
...
class ERPC_ThreeDApp extends React.PureComponent {
    constructor(props) {
        ...
        this.mainLoader = new Rhino3dmLoader();
        this.mainLoader.setLibraryPath('/vendor/other/');
    ...
    modelChanged() {
        ...
        let self = this;
        alert("this.state.drawing:" + this.state.drawing.name);
        alert("this.scene:" + this.scene);
        alert("this.state.inited:" + this.state.inited);
        if (this.state.showingModelPath != this.state.drawing.全局模型文件路径) {
            ...
            loader.load(window.location.origin + this.state.drawing.全局模型文件路径, function (object) {
                ...
                try {
                    ...
                    alert("self:" + self);
                    alert("scene:" + scene);
                    alert("inited:" + self.state.inited);
