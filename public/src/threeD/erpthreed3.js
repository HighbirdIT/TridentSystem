    ...
    图纸部位Changed(want图纸部位) {
        ...
        if (want图纸部位 == null) {
            ...
            for(let index=-1;index <2;++index){
                ...
                let 关联模型 = self.关联构件模型_dic[部位key];
                if(关联模型 != null){
            ...
            if(this.state.drawing.全局模型文件代码 == 0){
                ...
                extraUIs_arr.push(
                    <div key='toprightBtns' className="d-flex flex-column position-absolute " style={{ left: (canvasWidth - 5) + 'px', top: '5px', transform: 'translate(-100%,0%)' }}>
                        ...
                        <button onClick={this.toggleComponentModelVisible} className="btn btn-sm btn-light flex-grow-0 flex-shrink=0" ><i className={"fa fa-eye" + (this.state.comModelVisible == false ? '-slash text-danger' : '')}></i>构件模型</button>
