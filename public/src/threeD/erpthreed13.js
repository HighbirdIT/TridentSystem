...
class C构件上传记录{
    ...
    set(上传记录代码,构件编号, 构件全称, 文件路径, 上传时间, 构件代码,worldPos){
        this.上传记录代码 = 上传记录代码;
        this.构件编号 = 构件编号;
        this.构件全称 = 构件全称;
        this.上传时间 = 上传时间;
...
const componentModelMat = new THREE.MeshStandardMaterial({ color: 0x8030B, side: THREE.DoubleSide });
...
function shotParam(名称显示, 发图记录代码, 参数上传记录代码, completeCallBack) {
    ...
    popPersistentPage('M_Page_ParamShoter', ()=>{return <VisibleCM_Page_ParamShoter key='M_Page_ParamShoter' id='M_Page_ParamShoter' parenPath='M_Page_ParamShoter' />});
}
...
function shotComParam(构件代码,参数上传数据代码,图纸种类代码, completeCallBack) {
    ...
    popPersistentPage('M_Page_ParamShoter_com', ()=>{return <VisibleCM_Page_ParamShoter key='M_Page_ParamShoter_com' id='M_Page_ParamShoter_com' parenPath='M_Page_ParamShoter_com' />});
}
...
class CM_Page_ParamShoter extends React.PureComponent {
    ...
    renderContent() {
        ...
        retElem = (
            <div className='d-flex flex-grow-1 flex-shrink-1 flex-column fixPageContent '>
                <VisibleERPC_IFrame className='flex-grow-1 flex-shrink-1 d-flex flex-column ' proj='GJYM' flowCode='285' pageType='mb' onMessageFun={iframe_shot_onReceiveMsg} id='iframe_shot' parentPath={this.props.parenPath} />
                {this.renderSidePage()}
            </div>);
        return retElem;
    }
