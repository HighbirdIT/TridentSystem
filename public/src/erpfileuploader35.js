...
class ERPC_SingleFileUploader extends React.PureComponent {
    ...
    render() {
        ...
        return <div className={className} style={this.props.style} fixedsize={this.props.fixedsize}>
            <input onChange={this.fileSelectedHandler} ref={this.fileTagRef} type='file' className='d-none' accept={accept} capture={this.props.capture} />
...
class ERPC_FilePreview extends React.PureComponent {
    ...
    render() {
        ...
        if(this.props.filePath == null || this.props.filePath.length == 0){
            ...
            if (aidData.fileType == 'image') {
                contetnElem = <img onLoad={this.imgLoaded} className='w-100 h-100' src={window.location.origin + this.props.filePath} onClick={this.clickIconHandler} />
            }
            ...
            if(!this.canPreview){
                downloadBtn = <button onClick={this.clickIconHandler} type='button' className='btn btn-sm btn-primary'><i className={'fa fa-download'} /></button>;
            }
        }

        var useSyle = this.props.style;
        var rootDiv = this.rootDivRef.current;
        if(this.props.sizeMode == 'natural'){
            if(this.state.naturalWidth > 0){
                useSyle = Object.assign({},useSyle);
                var scale = isNaN(this.props.scale) ? 1 : this.props.scale;
                var width = this.state.naturalWidth * scale;
                var height = this.state.naturalHeight * scale;
                useSyle.width = width + 'px';
                useSyle.height = height + 'px';
            }
        }
        else if(this.props.sizeMode == 'fitWidth'){
            if(this.state.naturalWidth > 0 && rootDiv != null){
                useSyle = Object.assign({},useSyle);
                var scale = rootDiv.clientWidth/this.state.naturalWidth;
                var height = this.state.naturalHeight * scale;
                useSyle.height = height + 'px';
            }
        }
        else if(this.props.sizeMode == 'fitHeight'){
            if(this.state.naturalHeight > 0 && rootDiv != null){
                useSyle = Object.assign({},useSyle);
                var scale = rootDiv.clientHeight/this.state.naturalHeight;
                var width = this.state.naturalWidth * scale;
                useSyle.width = width + 'px';
            }
        }
        
        let canDelte = this.props.canDelte;
        if (fileName.length > 15) {
            fileName = '...' + this.props.fileName.substr(-15);
        }
        var divClassName = 'filepreview ' + (this.props.className ? this.props.className : '');
        
        return <div className={divClassName} fixedsize={this.props.fixedsize} style={useSyle} ref={this.rootDivRef} >
            {contetnElem}
            {
            ...
            <div id='toolbar' className='btn-group position-absolute' style={{right:'2px',bottom:'2px'}}>
                {downloadBtn}
                {!canDelte ? null : <button onClick={this.clickTrashHandler} id='del' className='btn btn-sm btn-danger'><i className={'fa ' + (this.deleting ? 'fa-circle-o-notch fa-spin' : 'fa-trash')} /></button>}
            </div>
        </div>
    }
}
...
function ERPC_FilePreview_mapstatetoprops(state, ownprops) {
    ...
    return {
        ...
        scale:isNaN(ctlState.scale) ? ownprops.scale : ctlState.scale,
    };
}
